...
<template>
  ...
  <div v-if="hasVideos || hasGraphics" class="flex gap-4 mb-8">
    <button v-if="hasVideos" @click="activeTab = 'videos'" :class="[
    ...
    <button v-if="hasGraphics" @click="activeTab = 'graphics'" :class="[
      'px-4 text-sm py-2 rounded-lg font-medium transition-colors',
  ...
  <div v-if="filteredContent.length > 0" class="grid grid-cols-4 gap-4 mb-8">
    <div v-for="(options, filter) in filters[activeTab]" :key="filter" class="space-y-2">
      ...
      <select v-model="currentFilters[filter]" class="w-full bg-zinc-800 rounded-lg px-3 py-2 !text-sm">
  ...
  <!-- Empty State -->
  <div v-if="!hasVideos && !hasGraphics" class="text-center py-12 text-zinc-400">
